...
time=$(date +%F@%T)
echo -e 删除6天前的日志...
find /root/firewall/ -mtime +30 -name '*.log' | xargs rm -f
echo -e 保存lastb日志...
lastb > /root/firewall/lastb-$time.log 
file=/root/firewall/$time.log
...
echo -e 发送邮件...
mail -a /root/firewall/lastb-$time.log -s "N1_firewall" vx@vxoice.onmicrosoft.com < $file
echo -e 清空btmp文件...
echo > /var/log/btmp
echo -e 更新防火墙规则...
firewall-cmd --reload
